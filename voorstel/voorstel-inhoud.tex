...
De eerste fase binnen dit proces in de reconnaissance-fase, waar er zo veel mogelijk data wordt verzamelt over het doelwit. Dit is een van de belangrijkste fases, 
aangezien deze fase de basis legt voor rest van de pentest. Als deze fase handmatig wordt uitgevoerd zal deze stap te veel tijd kosten. Met efficiëntie als doel zoekt 
dit onderzoek naar geautomatiseerde tools om het handmatige te vervangen.
Het doel van dit onderzoek is het vergelijken van de beschikbare tools en technieken om er de meest efficiënte uit te halen, weliswaar zonder kwaliteitsverlies van de verzamelde informatie.
De doelgroep van mijn onderzoek zijn IT’ers die werken met pentests, zoals cybersecurity- en IT-professionals en bedrijven die regelmatig hun systemen testen op kwetsbaarheden. 
Deze doelgroep is op zoek naar een betrouwbare tool dat efficiënt en een snelle werking heeft. Om dit te onderzoeken zal ik een literatuuronderzoek doen naar enkele bestaande tools zoals 
AutoRecon, Recon-ng en Shodan, aangevuld met testen op pentest scenario’s om deze werking te analyseren en hun efficiëntie te vergelijken. Het eindresultaat van dit 
onderzoek zal bestaan uit aanbevelingen voor de intigratie en werking van deze tools.
...
De reconnaissance-fase, of de verkenningsfase, is een fase binnen pentests die een cruciale rol heeft, maar vaak ook tijdrovend is. 
Deze fase is vaak de meest tijdrovende stap, het kan weken tot maanden 
duren, afhankelijk van de complexiteit van het target en de technieken die worden gebruikt~\autocite{Shah}. 
...
mogelijk informatie verzamelen van informatie. De pasieve methode is het verzalelen van het doelwit 
zonder direct contact met het doelwit, de actieve methode is wel direct
verbonden met het doelwit via tools om te scannen zoals Nmap, om open poorten te detecteren~\autocite{Shah}.

\subsection{Automatisering}

~\textcite{Hoang} heeft een geautomatiseerde aanpak voor pentesten in hij deep reinforcement 
...
gaan over het kiezen van de payloads de juiste kwetsbaarheden. Zijn methode is gericht op drie functies: informatie verzamelen, 
exploitatie en rapportage. Hoang toont daarmee aan dat het gebruik van deze aanpak niet enkel de presatie verbetert, maar ook dat het systeem de resultaten 
kan opslaan om die toe te passen op nieuwe situaties.~\autocite{Hoang}.

Volgens ~\textcite{Kothia} kan de eerste fase binnen pentesting in reconnaissance-face veel 
efficienter gebeuren met behulp van automatisatie. Er zijn veel open source tools beschikbaar, maar helaas vraagt het gebruik van deze 
tools nog veel handmatige inspanning om ze te intigreren. Kothia heeft een geautomatiseerde aanpak gecrieert om deze tools
...
Om penetratietests te automatiseren, zijn er verschillende frameworks en tools beschikbaar om 
efficiënt en snel informatie kan verzalelen. De meeste van deze tools zijn open source, waaronder 
...
en er de kwetsbaarheden uit kunnen halen en de kwedsbaarheden te kunnen detecteren. Door automatisatie kunnen 
organisaties sneller en kosten-effectiever tests uitvoeren. De automatisatie kan echter wel leiden tot een kwaliteitsverlies, 
...
Vaak wordt een keuze gemaake tussen een handmatige of een geautomatiseerde penettest, deze keuze hangt vaak af van de specefieke 
vereisten van de oranisatie. Echter wordt er ook niet gekozen om puur handmatig of geautomatiseerde pentest te maken, maar wordt
een hybryde model vaker gebruikt. Dit is vaak de ideale oplossing waarbij automatisatie en menselijke ervaring elkaar kunnen 
aanvullen voor een efficiënte Reconnaissance-fase~\autocite{techtarget}.

\subsection{Efficiëntie en uitdagingen}

Momenteel zijn er nog enkele uitdagingen bij penetests,zoals textcite{Fugkeaw} beschrijft, dat er nog te veel wordt

vertrouwd en testen afhankelijk zijn op de experten van het vak bij het beoordelen van de al dan niet geautomatiseerde kwetsbaarheid.
...
inconsistent door de individuele voorkeuren van de pentester volgenstextcite{Ghanem}.


Binnen pentesting worden er nog vaak nieuwe technologieën gebruikt, een van deze nieuwe technologieën is het gebruik van
...
Dit omvat dat er een workflow zal worden geïmplementeerd met behulp van de automatisatie tools zoals AutoRecon en Recon-ng. Deze
...
nauwkeurigheid en consistentie te meten. Met behulp van deze data kunnen we nieuwe technolgieent integreren om onze automatisatie te optimaliseren
...
\begin{enumerate}
    \item \textbf{Literatuurstudie} (3 weken): Relevante bronnen zal worden geanalyseerd en de onderzoeksresultaten worden geïdentificeerd.
...
zal zijn waar een er geavanceerde tool en frameworks zal gebruikt kunnen worden zoals AutoRecon, Shodan, Nmap en Recon-ng,
gecombineerd met menselijke expertise. Dit zal er voor zorgen dat er een snellere, betrouwbaardere en nauwkeurigere verzameling van informatie
